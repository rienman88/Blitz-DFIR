...
RUN_SUFFIX="${RUN_SUFFIX:-rocba_memory_no_llm}"
CASE_OBJECTIVE="${CASE_OBJECTIVE:-Analyze the Rocba memory image for evidence-backed suspicious processes, execution artifacts, persistence indicators, credential activity, and unknowns while avoiding unsupported conclusions.}"
...
FORCE="${FORCE:-0}"
RUN_MODE="${RUN_MODE:-clean}"
RESUME_SESSION="${RESUME_SESSION:-}"

RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)_${RUN_SUFFIX}"
if [[ "${RUN_MODE}" != "clean" && "${RUN_MODE}" != "resume" ]]; then
  echo "unsupported RUN_MODE=${RUN_MODE}; expected clean or resume" >&2
  exit 2
fi
if [[ "${RUN_MODE}" == "clean" ]]; then
  if [[ -n "${RESUME_SESSION}" ]]; then
    echo "RUN_MODE=clean cannot use RESUME_SESSION. Use RUN_MODE=resume for continuation." >&2
    exit 2
  fi
  RUN_ROOT="${CASE_ROOT}/analysis/runs/${RUN_ID}"
else
  if [[ -z "${RESUME_SESSION}" ]]; then
    echo "RUN_MODE=resume requires RESUME_SESSION=/cases/<CASE>/output/sess-..." >&2
    exit 2
  fi
...
PROCESS_PATTERN='app.py analyze|(^|[[:space:]/])vol([[:space:]]|$)|psort.py|log2timeline.py|psteal.py|case.plaso'
...
payload = {
    "schema_version": "blitz-memory-run-status.v1",
    "case_id": case,
    "run_id": run_id,
    "run_root": run_root,
    "status": status,
...
if status == "COMPLETED":
    ...
    payload["postrun_checks_reason"] = (
        "memory runner has no separate postrun QA phase; analysis validation, "
        "coverage, reporting, and artifact hashing completed inside the Blitz pipeline"
    )
if message:
...
[[ -n "${CASE_ROOT}" && "${CASE_ROOT}" != "/" ]] || fail "unsafe_case_root"
[[ "${MANIFEST}" != "/case.yaml" ]] || fail "manifest_resolved_to_root_case_yaml_set_CASE_or_CASE_ROOT"
...
if [[ -n "${ACTIVE_PROCESSES}" && "${FORCE}" != "1" ]]; then
  ...
  echo
  echo "refusing to start another memory run while analysis/tool work is active. Set FORCE=1 only after confirming this is safe." >&2
  exit 3
fi
...
write_run_status "RUNNING" "analysis_starting" "" "Memory runner preflight completed."
...
if [[ "${RUN_MODE}" == "resume" ]]; then
  ANALYZE_ARGS+=(--resume-session "${RESUME_SESSION}")
fi
...
if [[ "${ANALYSIS_EXIT}" == "0" ]]; then
  write_run_status "COMPLETED" "analysis_completed" "${ANALYSIS_EXIT}" "Memory analysis completed."
else
  write_run_status "FAILED" "analysis_failed" "${ANALYSIS_EXIT}" "Memory analysis exited non-zero."
fi
