...
EVIDENCE1_PATH="${EVIDENCE1_PATH:-${DATASET1_PATH:-}}"
EVIDENCE1_TYPE="${EVIDENCE1_TYPE:-${DATASET1_TYPE:-}}"
EVIDENCE1_ID="${EVIDENCE1_ID:-}"
EVIDENCE1_SHA="${EVIDENCE1_SHA:-}"
EVIDENCE1_SHA_FILE="${EVIDENCE1_SHA_FILE:-}"

EVIDENCE2_PATH="${EVIDENCE2_PATH:-${DATASET2_PATH:-}}"
EVIDENCE2_TYPE="${EVIDENCE2_TYPE:-${DATASET2_TYPE:-}}"
EVIDENCE2_ID="${EVIDENCE2_ID:-}"
EVIDENCE2_SHA="${EVIDENCE2_SHA:-}"
EVIDENCE2_SHA_FILE="${EVIDENCE2_SHA_FILE:-}"

ENABLE_REASONING="${ENABLE_REASONING:-0}"
CASE_OBJECTIVE="${CASE_OBJECTIVE:-Analyze the selected evidence for evidence-backed suspicious processes, execution artifacts, persistence indicators, credential activity, user activity, temporal gaps, cross-source correlation, and unknowns while avoiding unsupported conclusions.}"
...
FORCE="${FORCE:-0}"
RUN_SUFFIX="${RUN_SUFFIX:-external_evidence_clean}"
...
fail() {
  ...
  echo "evidence1_path=${EVIDENCE1_PATH:-unset}" >&2
  echo "evidence1_type=${EVIDENCE1_TYPE:-unset}" >&2
  echo "evidence2_path=${EVIDENCE2_PATH:-unset}" >&2
  echo "evidence2_type=${EVIDENCE2_TYPE:-unset}" >&2
...
payload = {
    "schema_version": "blitz-external-evidence-run-status.v1",
    "case_id": case,
    "run_id": run_id,
    "run_root": run_root,
    "status": status,
...
def request_json(url: str, payload: dict | None = None, timeout: int = 60) -> dict:
    ...
    request = urllib.request.Request(
        url,
        data=data,
        headers={"Authorization": "Bearer ollama", "Accept": "application/json", "Content-Type": "application/json"},
...
[[ -n "${EVIDENCE1_PATH}" ]] || fail "evidence1_path_required"
[[ -n "${EVIDENCE1_TYPE}" ]] || fail "evidence1_type_required"
[[ -f "${EVIDENCE1_PATH}" ]] || fail "evidence1_path_not_found"
if [[ -n "${EVIDENCE2_PATH}" || -n "${EVIDENCE2_TYPE}" ]]; then
  [[ -n "${EVIDENCE2_PATH}" ]] || fail "evidence2_path_required_when_type_set"
  [[ -n "${EVIDENCE2_TYPE}" ]] || fail "evidence2_type_required_when_path_set"
  [[ -f "${EVIDENCE2_PATH}" ]] || fail "evidence2_path_not_found"
fi

EVIDENCE1_PATH="$(absolute_path "${EVIDENCE1_PATH}")"
if [[ -n "${EVIDENCE2_PATH}" ]]; then
  EVIDENCE2_PATH="$(absolute_path "${EVIDENCE2_PATH}")"
fi

mkdir -p "${RUN_ROOT}" "${OUTPUT_DIR}" "${CASE_ROOT}/analysis/runs"

USES_MEMORY=0
if [[ "${EVIDENCE1_TYPE^^}" == "MEMORY" || "${EVIDENCE2_TYPE^^}" == "MEMORY" ]]; then
  USES_MEMORY=1
fi
if [[ "${USES_MEMORY}" == "1" ]]; then
  mkdir -p "${VOLATILITY_SYMBOLS_DIR}" || fail "volatility_symbols_dir_create_failed"
  touch "${VOLATILITY_SYMBOLS_DIR}/.blitz_write_probe" || fail "volatility_symbols_dir_not_writable"
...
if [[ -n "${ACTIVE_PROCESSES}" && "${FORCE}" != "1" ]]; then
  ...
  echo
  echo "refusing to start another Blitz run while analysis/tool work is active. Set FORCE=1 only after confirming this is safe." >&2
  exit 3
fi
...
if [[ -n "${FREE_GB}" && "${FREE_GB}" -lt "${RECOMMENDED_FREE_GB}" ]]; then
  ...
  echo "warning=raw_evidence_not_copied_but_outputs_need_space" >&2
fi

EVIDENCE1_HASH="$(resolve_hash "${EVIDENCE1_PATH}" "${EVIDENCE1_SHA}" "${EVIDENCE1_SHA_FILE}")"
echo "${EVIDENCE1_HASH}  ${EVIDENCE1_PATH}" | sha256sum -c -
MANIFEST_ARGS=("${MANIFEST}" "${CASE}" "${OUTPUT_DIR}" "${EVIDENCE1_ID}" "${EVIDENCE1_TYPE}" "${EVIDENCE1_PATH}" "${EVIDENCE1_HASH}")

if [[ -n "${EVIDENCE2_PATH}" ]]; then
  EVIDENCE2_HASH="$(resolve_hash "${EVIDENCE2_PATH}" "${EVIDENCE2_SHA}" "${EVIDENCE2_SHA_FILE}")"
  echo "${EVIDENCE2_HASH}  ${EVIDENCE2_PATH}" | sha256sum -c -
  MANIFEST_ARGS+=("${EVIDENCE2_ID}" "${EVIDENCE2_TYPE}" "${EVIDENCE2_PATH}" "${EVIDENCE2_HASH}")
fi

"${PYTHON}" - "${MANIFEST_ARGS[@]}" <<'PY'
from __future__ import annotations
...
items = sys.argv[4:]
if len(items) % 4:
    raise SystemExit("invalid evidence arguments")

used_ids: set[str] = set()
evidence = []
for index in range(0, len(items), 4):
    raw_id, evidence_type, path, digest = items[index : index + 4]
    if raw_id:
        evidence_id = raw_id
    else:
        stem = Path(path).stem.lower()
        evidence_id = re.sub(r"[^a-z0-9_.-]+", "-", stem).strip("-._") or f"evidence-{index // 4 + 1}"
    original = evidence_id
    counter = 2
    while evidence_id in used_ids:
        evidence_id = f"{original}-{counter}"
        counter += 1
    ...
    evidence.append(
        {
            ...
            "sha256": digest,
            "description": "User-selected evidence referenced in place; raw evidence is not copied into the Blitz case.",
        }
    )
...

{
  ...
  echo "evidence1_path=${EVIDENCE1_PATH}"
  echo "evidence1_type=${EVIDENCE1_TYPE}"
  echo "evidence2_path=${EVIDENCE2_PATH:-none}"
  echo "evidence2_type=${EVIDENCE2_TYPE:-none}"
...
write_run_status "RUNNING" "analysis_starting" "" "External evidence runner preflight completed."
...
if [[ "${ANALYSIS_EXIT}" == "0" ]]; then
  write_run_status "COMPLETED" "analysis_completed" "${ANALYSIS_EXIT}" "External evidence analysis completed."
else
  write_run_status "FAILED" "analysis_failed" "${ANALYSIS_EXIT}" "External evidence analysis exited non-zero."
fi
